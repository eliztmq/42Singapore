...
/*   Updated: 2024/02/27 22:27:52 by elizabethte      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_point(t_coord *pt, t_quat q)
{
	...
	rotated_pt.x = (q.w * q.w * pt->x) + (2 * q.y * q.w * pt->z)
		- (2 * q.z * q.w * pt->y) + (q.x * q.x * pt->x) + (2 * q.y * q.x * pt->y)
		+ (2 * q.z * q.x * pt->z) - (q.z * q.z * pt->x) - (q.y * q.y * pt->x);
	rotated_pt.y = (2 * q.x * q.y * pt->x) + (q.y * q.y * pt->y)
		+ (2 * q.z * q.y * pt->z) + (2 * q.w * q.z * pt->x) - (q.z * q.z * pt->y)
		+ (q.w * q.w * pt->y) - (2 * q.x * q.w * pt->z) - (q.x * q.x * pt->y);
